...
import { Car as CarIcon, Plus, Search, MapPin } from 'lucide-react';
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function Page() {
  ...
  const onSubmit = async (data: CreateCarInput) => {
    try {
      ...
      await (createCar.mutateAsync as (vars: CreateCarInput) => Promise<unknown>)(data);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900">Novo Carro</h1>
        </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              <div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="cod" className="text-slate-700">Código *</Label>
                    <Input
                      id="cod"
                      {...form.register("cod")}
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="mark" className="text-slate-700">Marca *</Label>
                    <Input
                      id="mark"
                      {...form.register("mark")}
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="capacity" className="text-slate-700">Capacidade *</Label>
                    <Input
                      id="capacity"
                      type="number"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="companyId" className="text-slate-700">Empresa *</Label>
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        variant="outline"
                        className="rounded-lg"
                        onClick={() => {/* Abrir modal para adicionar empresa */}}
                      >
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="containerId" className="text-slate-700">Container *</Label>
                    <div className="flex gap-2">
                      <Select
                        value={form.watch("containerId")}
                        onValueChange={(value) => form.setValue("containerId", value)}
                      >
                        <SelectTrigger className="rounded-lg w-full">
                      ...
                      <Button
                        ...
                        variant="outline"
                        className="rounded-lg"
                        onClick={() => {/* Abrir modal para adicionar container */}}
                      >
                        <Plus className="w-4 h-4" />
                      </Button>
                    </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Select
                      ...
                      onValueChange={(value) => form.setValue("geoLocationEntityId", value)}
                    >
                      <SelectTrigger className="rounded-lg w-full">
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="rounded-lg"
                      onClick={() => {/* Abrir modal para adicionar geolocalização */}}
                    >
                      <Plus className="w-4 h-4" />
                    </Button>
                  </div>
                ...
                <div className="space-y-2">
                    <Label htmlFor="status" className="text-slate-700">Ativo *</Label>
                    <div className="flex items-center gap-3 py-2">
                      <Switch id="status" checked={!!form.watch('status')} onCheckedChange={(v) => form.setValue('status', v)} className="cursor-pointer" />
                      <span className="text-slate-600 text-sm">{form.watch('status') ? 'Ativo' : 'Inativo'}</span>
                    </div>
                    {form.formState.errors.status && (
                      <p className="text-sm text-red-500">{form.formState.errors.status.message as string}</p>
                    )}
                  </div>
            </div>
            </div>
            ...
            <div className="bg-slate-50 px-8 py-4 flex justify-end gap-3 border-t border-slate-200">
              ...
              >
                {isSubmitting ? "Salvando..." : "Criar Carro"}
              </Button>
            </div>
          </div>
        </form>
