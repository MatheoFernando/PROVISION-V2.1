        ...
        <CardContent>
          <div className="grid gap-4 grid-cols-2 lg:grid-cols-3">
            {defaultServices.map((service, index) => {
              ...
              return (
                <div key={index} className={`border rounded-lg p-4 hover:bg-accent ${disabled ? 'opacity-60' : ''}`}>
                  <Link href={service.url} className={disabled ? 'pointer-events-none' : ''}>
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        <div className="rounded-lg p-2 bg-accent/10">
                          <Image src={service.imageSrc} alt={service.name} width={32} height={32} className="h-8 w-8 object-contain" />
                        </div>
                        <h3 className="font-semibold">{service.name}</h3>
                      </div>
                      <Badge variant={disabled ? 'secondary' : 'default'} className={disabled ? '' : 'bg-green-500'}>
